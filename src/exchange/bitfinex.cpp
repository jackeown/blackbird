...
double getQuote(Parameters& params, bool isBid) {
  ...
  json_t* root = getJsonFromUrl(params, "https://api.bitfinex.com/v1/ticker/ltcusd", "", GETRequest);
  const char* quote;
  double quoteValue;
  if (isBid) {
...
double getLimitPrice(Parameters& params, double volume, bool isBid) {
  ...
  bool GETRequest = true;
  if (isBid) {
    root = json_object_get(getJsonFromUrl(params, "https://api.bitfinex.com/v1/book/ltcusd", "", GETRequest), "bids");
  } else {
    root = json_object_get(getJsonFromUrl(params, "https://api.bitfinex.com/v1/book/ltcusd", "", GETRequest), "asks");
  }
  *params.logFile << "<Bitfinex> Looking for a limit price to fill " << fabs(volume) << " LTC..." << std::endl;
  double tmpVol = 0.0;
  double p;
  double v;
  int i = 0;
