...
double getQuote(Parameters& params, bool isBid) {
  ...
  json_t* root = getJsonFromUrl(params, "https://api.bitfinex.com/v1/ticker/ethusd", "", GETRequest);
  const char* quote;
  double quoteValue;
  if (isBid) {
...
int sendOrder(Parameters& params, std::string direction, double quantity, double price) {
  *params.logFile << "<Bitfinex> Trying to send a \"" << direction << "\" limit order: " << quantity << "@$" << price << "..." << std::endl;
  ...
  oss << "\"symbol\":\"ethusd\", \"amount\":\"" << quantity << "\", \"price\":\"" << price << "\", \"exchange\":\"bitfinex\", \"side\":\"" << direction << "\", \"type\":\"limit\"";
...
double getLimitPrice(Parameters& params, double volume, bool isBid) {
  ...
  bool GETRequest = true;
  if (isBid) {
    root = json_object_get(getJsonFromUrl(params, "https://api.bitfinex.com/v1/book/ethusd", "", GETRequest), "bids");
  } else {
    root = json_object_get(getJsonFromUrl(params, "https://api.bitfinex.com/v1/book/ethusd", "", GETRequest), "asks");
  }
  *params.logFile << "<Bitfinex> Looking for a limit price to fill " << fabs(volume) << " ETH..." << std::endl;
  double tmpVol = 0.0;
  double p;
  double v;
  int i = 0;
