...
double getQuote(Parameters& params, bool isBid) {
  ...
  if (krakenGotTicker) {
    ...
    root = getJsonFromUrl(params, "https://api.kraken.com/0/public/Ticker", "pair=XLTCZUSD", GETRequest);
  ...
  double quoteValue;
  if (isBid) {
    quote = json_string_value(json_array_get(json_object_get(json_object_get(json_object_get(root, "result"), "XLTCZUSD"), "b"), 0));
  } else {
    quote = json_string_value(json_array_get(json_object_get(json_object_get(json_object_get(root, "result"), "XLTCZUSD"), "a"), 0));
  }
  if (quote != NULL) {
...
double getAvail(Parameters& params, std::string currency) {
  ...
  if (currency.compare("usd") == 0) {
    ...
    const char * avail_str = json_string_value(json_object_get(result, "XLTC"));
    available = avail_str ? atof(avail_str) : 0.0;
  } else {
...
double getLimitPrice(Parameters& params, double volume, bool isBid) {
  ...
  if (krakenGotLimPrice) {
    ...
    root = json_object_get(json_object_get(getJsonFromUrl(params, "https://api.kraken.com/0/public/Depth", "pair=XLTCZUSD", GETRequest), "result"), "XLTCZUSD");
